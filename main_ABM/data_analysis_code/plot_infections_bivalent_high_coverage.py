...
def plot_ribbon_infections_over_time_plus(younger_or_older=["older"],immune_escape_time=546,bivalent_start_time=546):
    max_infections=5000 
    ...
    for boosting_time in boosters_only_vaccination_start_list:
        ...
        for local_TP_list in TP_segregated_list:
            ...
            legend0.append(ax.scatter(-10000,-10000,color=bivalent_colour_general, s=100, marker= marker, alpha=1.0, edgecolors='none'))
            ...
            if boosting_time >=bivalent_start_time:
                ax2.axvspan(boosting_time,boosting_time+13*7,facecolor= bivalent_colour_general,zorder=0)
            else:
...
def total_deaths_histograms(boosting_time,immune_escape_time,bivalent_start_time,ICU_or_death='death',younger_or_older=["older"],timeframe =local_days,minimum_age = 0):
    BA1_colour = '#a1a1a1'
    BA45_colour = "#666666"
    vaccinating_colour = "yellowgreen"
    boosting_colour_general = "darkgreen"
    ...
    for local_TP_list in TP_segregated_list:
        ...
        legend1.append(ax4.scatter(-10000,-10000,color=bivalent_colour_general, s=100, marker= marker, alpha=1.0, edgecolors='none'))
        ax4.axvspan(min(local_days),546,facecolor= vaccinating_colour,zorder=0)
        if boosting_time>=bivalent_start_time:
            ax4.axvspan(boosting_time,boosting_time+13*7,facecolor= bivalent_start_time,zorder=0)
        else:
        ...
        ax4.legend(legend1, ["main vaccinaton program","further boosting program","bivalent boosters"],bbox_to_anchor=(1.01,-0.2), loc="lower left",borderaxespad=0,frameon=False)
...
def plot_ribbon_infections_over_time_plus_fixed_boosting_group(boosting_group_here = "65+",younger_or_older=["older"],immune_escape_time=546):
    ...
    for local_TP_list in TP_segregated_list:
        for population_type in younger_or_older:
            ...
            ax2.legend(legend0, ["circulating BA.1","circulating BA.4/5", "main vaccination program"],bbox_to_anchor=(1.01,0), loc="lower left",borderaxespad=0,frameon=False)

            if population_type=="older":
                leg = Legend(ax, legend_points,[str(round(x/(52*7),2)) + " years"  for x in boosters_only_vaccination_start_list],title=younger_or_older[0] +" population\nhigh risk boosting (65+)\nfurther boosting commences at:",bbox_to_anchor=(1.01,0), loc="lower left",borderaxespad=0,frameon=False)
            ...
            plt.savefig(os.path.join(folder, "ribbon_infections_over_time_plus_fixed_boosting_group"+younger_or_older[0]+ "_boosting_"+str(boosting_group_here)+ "_maxTP_"+str(max(local_TP_list)) + ".png") , bbox_inches='tight')
            plt.close()
...
def total_deaths_histograms_fixed_boosting_group(immune_escape_time,boosting_group_here = "65+",ICU_or_death='death',younger_or_older=["older"],timeframe =local_days,minimum_age = 0):
    ...
    for local_TP_list in TP_segregated_list:
        for population_type in younger_or_older:
            ...
            plt.savefig(os.path.join(folder, "total_" + ICU_or_death+"_histogram_"+population_type+ "_fixed_boosting_group"+"_boosting_"+str(boosting_group_here)+ "_maxTP_"+str(max(local_TP_list)) +"_time"+str(timeframe[0])+"-" + str(timeframe[-1])+"_minimum-age-"+str(minimum_age)+ ".png") , bbox_inches='tight')
            plt.close()
...
#     for immune_escape_time in [original_program_time]:# immune_escape_times:
...
#         for boosting_time in [boosters_only_vaccination_start_list[0]]:
...
#             total_deaths_histograms(boosting_time,immune_escape_time,ICU_or_death='death',younger_or_older=younger_or_older,timeframe=list(range(original_program_time,max_days)),minimum_age = 0)
            ...
            # plot_before_vs_after_infections(boosting_time,population_type_list = younger_or_older,x_limits=[0,100],y_limits = [0,200],aspect_ratio = 'equal',cut_time=original_program_time)
            # plot_before_vs_after_infections(boosting_time,population_type_list = younger_or_older,x_limits=[0,200],y_limits = [0,200],aspect_ratio = 'equal',cut_time=boosting_time)


            # total_infections_and_deaths_histograms(boosting_time,ICU_or_death='death',younger_or_older=younger_or_older,timeframe=local_days)
            # total_infections_and_deaths_histograms(boosting_time,ICU_or_death='death',younger_or_older=younger_or_older,timeframe=list(range(boosting_time,max_days)))
            # total_infections_and_deaths_histograms(boosting_time,ICU_or_death='death',younger_or_older=younger_or_older,timeframe=list(range(original_program_time,max_days)))

            # total_infections_and_deaths_histograms(boosting_time,ICU_or_death='death',younger_or_older=younger_or_older,timeframe=local_days,minimum_age=65)
            # total_infections_and_deaths_histograms(boosting_time,ICU_or_death='death',younger_or_older=younger_or_older,timeframe=list(range(boosting_time,max_days)),minimum_age=65)
        ...
        # total_infections_and_deaths_histograms_fixed_boosting_group(boosting_group_here = "65+",ICU_or_death='death',younger_or_older=younger_or_older,timeframe =local_days)
        # total_infections_and_deaths_histograms_fixed_boosting_group(boosting_group_here = "65+",ICU_or_death='death',younger_or_older=younger_or_older,timeframe =list(range(original_program_time,max_days)))

    # plot_before_vs_after_infections_ALL(population_type_list = younger_or_older,x_limits=[0,230],y_limits = [0,160],aspect_ratio = 'equal',infections_or_deaths_plotting="infections")
    # plot_before_vs_after_infections_ALL(population_type_list = younger_or_older,x_limits=[0,230],y_limits = [0,72],aspect_ratio = 'equal',infections_or_deaths_plotting="deaths")
    # plot_before_vs_after_infections_ALL_animated(population_type_list =younger_or_older,x_limits=[0,230],y_limits = [0,160],aspect_ratio = 'equal',infections_or_deaths_plotting="infections")
    # plot_before_vs_after_infections_ALL_animated(population_type_list = younger_or_older,x_limits=[0,230],y_limits = [0,72],aspect_ratio = 'equal',infections_or_deaths_plotting="deaths")

# results to match Natalie's
# for younger_or_older in  [["younger"], ["older"]]:
#     if younger_or_older == ["younger"]:
#         no_boosting_colour = 'white'
#         pedatric_boosting_colour = 'lightskyblue'
...
TP_segregated_list = [TP_high,TP_low] # just TP high
for younger_or_older in  [["older"],["younger"]]:
    ...
    for immune_escape_time in [original_program_time, original_program_time + 52*7]:
        ...
        folder = "/scratch/cm37/tpl/bivalent_boosting/high_coverage_immune_escape_t" + str(immune_escape_time) +"_bivalent_t"+str(bivalent_start_time) +"_outputs/"

        presim_parameters_folder  = '/fs04/cm37/prod/Le/WHO/covid-abm-presim/parameter_files_annual_boosting_1/'

        plot_ribbon_infections_over_time_plus(younger_or_older=younger_or_older,immune_escape_time=immune_escape_time,bivalent_start_time=bivalent_start_time)

        for boosting_time in [original_program_time + 26*7 ]:
            total_deaths_histograms(boosting_time,immune_escape_time,bivalent_start_time,ICU_or_death='death',younger_or_older=younger_or_older,timeframe=list(range(original_program_time,max_days)),minimum_age = 0)