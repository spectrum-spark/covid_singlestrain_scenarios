...
boosting_time = 728 # aka at two years
...
def get_iterated_clinical_DF(population_type, paramNum,folder,TP_val,days_all,boosting_scenario):
    ...
    iterated_clinical_DF.insert(0,'scenario',boosting_group_names[boosting_scenario])
    ...
    return iterated_clinical_DF


def get_combined_clinical_DF_by_summary_type(summary_type,iterated_clinical_DF):
    print("///////////////////////////////////////////")
    print(summary_type)
...
for days_all, days_name in zip(days_list,days_list_name):
    ...
    mean_output_file_name =  "bivalent_low_coverage_mean_clinical_outcomes_totals"+days_name+".csv"
    median_output_file_name =  "bivalent_low_coverage_median_clinical_outcomes_totals"+days_name+".csv"
    quantile025_output_file_name =  "bivalent_low_coverage_quantile_0.025_clinical_outcomes_totals"+days_name+".csv"
    quantile975_output_file_name =  "bivalent_low_coverage_quantile_0.975_clinical_outcomes_totals"+days_name+".csv"
    ...
    for TP_type, TP_val in zip(TP_type_list,TP_list):
        ...
        for population_type in ["younger"]:
            ...
            for paramNum in no_vax_list:
                iterated_clinical_DF = get_iterated_clinical_DF(population_type, paramNum,monovalent_folder,TP_val,days_all,'none')     

                mega_DF_list_iterated.append(iterated_clinical_DF)

                for summary_type in ["mean","median",0.025,0.975]:
                    
                    combined_clinical_DF = get_combined_clinical_DF_by_summary_type(summary_type,iterated_clinical_DF)
                    mega_DF_list[summary_type].append(combined_clinical_DF)

            ################################################
            # next is monovalent 
            for paramNum in param_list:
                iterated_clinical_DF = get_iterated_clinical_DF(population_type, paramNum,monovalent_folder,TP_val,days_all,'monovalent')     
            ...
            # finally is bivalent
            for paramNum in param_list:
                iterated_clinical_DF = get_iterated_clinical_DF(population_type, paramNum,bivalent_folder,TP_val,days_all,'bivalent')     
