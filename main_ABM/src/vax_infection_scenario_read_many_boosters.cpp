...
static void create_individuals_from_input_many_boosters(std::stringstream &individual_group, std::vector<Individual> &residents, std::vector<std::uniform_real_distribution<double>> &generate_age, std::vector<double> &age_brackets, nlohmann::json &ve_params)
{
    ...
    double time_booster_3;
    double time_booster_4;
    size_t infection;
 ...
 // Assign values: third (first booster) dose day
    ...
    // Assign values: fifth (third booster) dose day
    std::getline(individual_group, string_value, ',');
    if (string_value.empty())
    {
        // What do we do for empty stuff.
        time_booster_3 = -1.0; //shouldn't actually be empty though, should have filler values
    }
    else
    {
        string_value_stream = std::stringstream(string_value);
        string_value_stream >> time_booster_3;
    }
    // Assign values: sixth (fourth booster) dose day
    std::getline(individual_group, string_value, ',');
    if (string_value.empty())
    {
        // What do we do for empty stuff.
        time_booster_4 = -1.0; //shouldn't actually be empty though, should have filler values
    }
    else
    {
        string_value_stream = std::stringstream(string_value);
        string_value_stream >> time_booster_4;
    }
...
std::vector<Individual> read_individuals_from_input_many_boosters(std::string vaccination_infection_filename, std::vector<std::uniform_real_distribution<double>> &generate_age, std::vector<double> &age_brackets, nlohmann::json &ve_params)
{
    ...
    if (vaccination_infection_read.is_open())
    {
        ...
        while (std::getline(vaccination_infection_read, line))
        {
            ...
            create_individuals_from_input_many_boosters(individuals_stream, residents, generate_age, age_brackets, ve_params); // Create a group at a time.
        }
    }
    else
    {
