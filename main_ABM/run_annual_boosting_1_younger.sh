        ...
        do
            ...
            do
                ...
                do
                    ...
                    ./Run_BA1s_BA45s_cont_intros updated_omicron_parameters/omicron_May_2022_updates.json updated_omicron_parameters/transmission_escape_params.json simulation_params/annual_boosting_1_younger_immune_escape_t${BA45start}/sim_params_${POP}_${TPvers}.json $i ../presim_code/parameter_files_annual_boosting_1_younger abm_continuous_simulation_parameters_${POP}_${diffparams}


                    matlab -batch "cd '../clinical_pathways';disp(pwd);sev_mat = [1/15,1/45,1/60];TP_list = {'1.05','1.95'};TP = TP_list{${TPvers}};disp(TP);population = '${POP}';disp(population);params = '${diffparams}';disp(params);simnum='${i}';disp(simnum);BA45wavestart = '${BA45start}';foldername = strcat('../outputs/annual_boosting_1_younger_immune_escape_t',BA45wavestart,'/');disp(foldername);filename = strcat(foldername,'abm_continuous_simulation_parameters_',population,'_',params,'_SOCRATES_TP',TP,'/sim_number_',simnum);disp(filename);clinical_pathways_immunity_relsev3_func('NSW',filename,sev_mat);disp('done?');quit()"
...
python data_analysis_code/checking_convergence/plot_means_over_simulations_annual_boosting_1_younger.py # for checking convergence
