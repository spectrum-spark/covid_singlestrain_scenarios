...
def plot_separated_infections_over_time(younger_or_older=["younger"]):
    ...
    plot_save =["no_more_vax","reactive_vax"]
...
def plot_ribbon_infections_over_time(younger_or_older=["younger"],immune_escape_wave_start=2,seeding_or_continuous="seeding",with_vaccination_hatch=False):
    ...
    for i_third_vax in range(len(third_vaccination_type_list)):
        ...
        for local_TP_list in TP_segregated_list:
            ...
            if immune_escape_wave_start==2:
                plt.axvspan(second_exposure_time,max_days,facecolor="#666666",zorder=0)
            else:
                plt.axvspan(third_exposure_time,max_days,facecolor="#666666",zorder=0)

            if with_vaccination_hatch:
                # ax.fill_between([min(local_days),546],[max_infections,max_infections], facecolor="none", hatch="X", edgecolor="orchid", linewidth=0.0)
                plt.axvspan(min(local_days),546,facecolor="none",edgecolor= "forestgreen",zorder=0,hatch='/')
                if third_vaccination_type_here== "no further vaccination":
                    pass
                elif third_vaccination_type_here== "reactive (delayed) vaccination":
                    # ax.fill_between([third_exposure_time+14,third_exposure_time+14+150],[0,max_infections], facecolor="none", hatch="X", edgecolor="orchid", linewidth=0.0)
                    plt.axvspan(third_exposure_time+14,third_exposure_time+14+150,facecolor="none",edgecolor= "forestgreen",zorder=0,hatch='/')
...
def plot_ribbon_infections_over_time_plus(younger_or_older=["younger"],immune_escape_wave_start=2,seeding_or_continuous="seeding"):
    ...
    for i_third_vax in range(len(third_vaccination_type_list)):
        ...
        for local_TP_list in TP_segregated_list:
            ...
            ax.set_xticklabels(months_numbering)
            # ax.set_xlabel('time (months-ish)')
            ax2.set_xlabel('time (months-ish)')
            ...
            if immune_escape_wave_start==2:
                ax.axvspan(second_exposure_time,max_days,facecolor=BA45_colour ,zorder=0)
            else:
                ax.axvspan(third_exposure_time,max_days,facecolor=BA45_colour ,zorder=0)

            ax2.axvspan(min(local_days),546,facecolor= vaccinating_colour,zorder=0)
            if third_vaccination_type_here== "no further vaccination":
                pass
            elif third_vaccination_type_here== "reactive (delayed) vaccination":
                ax2.axvspan(third_exposure_time+14,third_exposure_time+14+150,facecolor= vaccinating_colour,zorder=0)
            ax2.set_yticklabels([])

            ax2.legend(legend0, ["circulating BA.1","circulating BA.4/5", "vaccination occuring"],bbox_to_anchor=(1.01,0), loc="lower left",borderaxespad=0,frameon=False)

            leg = Legend(ax, legend_points, ["no vaccination","20\% vaccination coverage", "50\% vaccination coverage","80\% vaccination coverage"],title=younger_or_older[0] +" population",bbox_to_anchor=(1.01,0), loc="lower left",borderaxespad=0,frameon=False)
            leg._legend_box.align = "left"
            ax.add_artist(leg)
            ...
            plt.savefig(os.path.join(folder, "ribbon_infections_over_time_plus_"+younger_or_older[0]+ "_"+plot_save[i_third_vax]+ "_maxTP_"+str(max(local_TP_list)) + ".png") , bbox_inches='tight')
            plt.close()
...
def plot_before_vs_after_infections_combined_ages_80_booster_only_horizontal(population_type_list = ["younger","older"],x_limits=[15,85],y_limits = [-1,60],filter=False,aspect_ratio = 'equal',past_waves=[1],future_waves=[2]):
    ...
    if future_waves ==[2]:
        ...
        list_of_param_list = [[0,1,3,5],[0,2,4,6]]
        third_vaccination_type = ["no further vaccination",  "reactive (delayed) vaccination"]
        plot_save =["no_more_vax","reactive_vax"]
...
def plot_third_wave_ICU_and_deaths_given_second_and_first_wave(ICU_or_death,OG="",population_type_list = ["younger","older"],ylimits = [0,28],y_ticks = list(range(0,29,2)),x_limits=[15,85]): # or "OG_"
    ...
    for first_wave_vax in first_wave_vax_coverage:
        ...
        for lower_attack,higher_attack in first_wave_attack_rate_limits:
            ...
            ax.legend(legend_points, ["no vaccination ever","no further vaccination", "reactive (delayed) vaccination"],title=population_type_list[0] +" population with \n" + str(first_wave_vax*100) +"\% vaccination in first year and \n"+str(lower_attack)+"\% - " + str(higher_attack)+"\% first year attack rate",bbox_to_anchor=(0.9, 0.9), loc=1)
    ...
    for population_type in population_type_list :
        ...
        for TP_type in TP_type_list:
            ...
            for vax_cov in [0.2,0.5,0.8]:
                ...
                for third_wave_vax_type in third_vaccination_type_list:
        ...
        for wavestart, folder_start, presim_parameters_folder_start in zip(wavestart_list,folder_start_list,presim_parameters_folder_start_list):
            for TP_type, local_TP_list in zip(TP_type_list,TP_reduced_list):
                ...
                for paramNum in param_list:
                    ...
                    boosters_only_vaccination_start = presim_parameters['boosters_only_vaccination_start']
                    third_vaccination_type = third_vaccination_type_list[boosters_only_vaccination_start_list.index(boosters_only_vaccination_start)]
                    if total_vaccination_rate in first_wave_vax_coverage:
                        pass 
                    else:
                        continue
...
def total_infections_and_deaths_histograms(ICU_or_death='death',younger_or_older=["younger"]):
    ...
    for i_third_vax in range(len(third_vaccination_type_list)):
        ...
        for local_TP_list in TP_segregated_list:
            ...
            for population_type in younger_or_older:
                ...
                # if population_type=="younger":
                #     legend_points.append(ax.scatter(-10000,-10000,color='white', s=100, marker= marker, alpha=1.0, edgecolors='lightskyblue'))
                ...
                # if population_type=="older":
                #     legend_points.append(ax.scatter(-10000,-10000,color='white', s=100, marker= marker, alpha=1.0, edgecolors='salmon'))
                ...
                total_infections_local_days = {0:[], 0.2: [],0.5:[],0.8 :[]}
                total_severe_disease_local_days = {0:[], 0.2: [],0.5:[],0.8 :[]}
                ...
                for paramNum in param_list:
                    for TP in local_TP_list:
                        ...
                        for simnum in df_dict.keys():
                            infections_over_time = df_dict[simnum]
                            total_infections = sum(list_conversion_nans(infections_over_time, local_days))
                            total_infections_local_days[total_vaccination_rate].append(total_infections ) 

                            for aug in range(1,aug_num+1):
                                new_pd_ICU = clinical_pd_obj.loc[(clinical_pd_obj['iteration']==(simnum-1)*aug_num+aug) & (clinical_pd_obj['day'].isin(local_days))]
                                
                                if ICU_or_death == 'death':
                                    daily_deaths = sum(new_pd_ICU['daily_deaths'].to_list())
                                    total_severe_disease_local_days[total_vaccination_rate].append(daily_deaths) 
                                elif ICU_or_death =='ICU':
                                    daily_ICU_admissions = sum(new_pd_ICU['daily_ICU_admissions'].to_list())
                                    total_severe_disease_local_days[total_vaccination_rate].append(daily_ICU_admissions) 
                ...
                for vax in [0,0.2,0.5,0.8]:
                    ...
                    ax.hist(total_infections_local_days[vax], bins=40, alpha=0.5, color=plot_colour)

                    ax2.hist(total_severe_disease_local_days[vax],bins=50, alpha=0.5, color=plot_colour)
            ...
            ax.set_xlabel("Total infections (900 days)")
            # ax.set_xlim(0,200000)

            ax2.set_ylabel('Count')
            ax2.set_xlim(0,140)
            if ICU_or_death == 'death':
                ax2.set_xlabel("Total deaths (900 days)")
            elif ICU_or_death =='ICU':
                ax2.set_xlabel("Total ICU Admissions (900 days)")
            ...
            plt.savefig(os.path.join(folder, "total_infections_and_" + ICU_or_death+"_histogram_"+"_".join(younger_or_older)+ "_"+plot_save[i_third_vax]+ "_maxTP_"+str(max(local_TP_list)) + ".png") , bbox_inches='tight')
            plt.close()


def wave_3_only_infections_and_deaths_histograms(ICU_or_death='death',younger_or_older=["younger"]):

    third_vaccination_type_list = ["no further vaccination", "reactive (delayed) vaccination"]
    plot_save =["no_more_vax","reactive_vax"]

    local_days = list(range(third_exposure_time,max_days))
    ...
    for i_third_vax in range(len(third_vaccination_type_list)):
        ...
        for local_TP_list in TP_segregated_list:
            ...
            ax.set_xlabel("infections (days 675-900)")
            # ax.set_xlim(0,200000)

            ax2.set_ylabel('Count')
            ax2.set_xlim(0,60)
            if ICU_or_death == 'death':
                ax2.set_xlabel("deaths (days 675-900)")
            elif ICU_or_death =='ICU':
                ax2.set_xlabel("ICU Admissions (days 675-900)")
            ...
            plt.savefig(os.path.join(folder, "wave_3_only_infections_and_" + ICU_or_death+"_histogram_"+"_".join(younger_or_older)+ "_"+plot_save[i_third_vax]+ "_maxTP_"+str(max(local_TP_list)) + ".png") , bbox_inches='tight')
            plt.close()

def comparing_reactive_vaccination_on_infections_and_deaths_histograms(ICU_or_death='death',younger_or_older=["younger"]):
    ...
    for population_type in younger_or_older:
        ...
        for vaccination_here in [0.2,0.5,0.8]:
            ...
            for local_TP_list in TP_segregated_list:
                ...
                total_infections_wave_3 = {"no further vaccination":[], "reactive (delayed) vaccination" :[]}
                total_severe_disease_wave_3 = {"no further vaccination":[], "reactive (delayed) vaccination" :[]}
                ...
                for paramNum in param_list:
                    ...
                    if total_vaccination_rate!=vaccination_here:
                        continue
                    ...
                    for TP in local_TP_list:
                        ...
                        for simnum in df_dict.keys():
                            infections_over_time = df_dict[simnum]
                            total_infections = sum(list_conversion_nans(infections_over_time, days_wave_3))
                            total_infections_wave_3[third_vaccination_type].append(total_infections ) 
                            ...
                            for aug in range(1,aug_num+1):
                                ...
                                if ICU_or_death == 'death':
                                    daily_deaths = sum(new_pd_ICU['daily_deaths'].to_list())
                                    total_severe_disease_wave_3[third_vaccination_type].append(daily_deaths) 
                                elif ICU_or_death =='ICU':
                                    daily_ICU_admissions = sum(new_pd_ICU['daily_ICU_admissions'].to_list())
                                    total_severe_disease_wave_3[third_vaccination_type].append(daily_ICU_admissions) 
                
                for third_vaccination_type in third_vaccination_type_list:
                    outline = 'none'
                    if population_type == "younger":
                        if vaccination_here == 0.2:
                            plot_colour = 'lightskyblue'
                            outline= 'lightskyblue'
                        elif vaccination_here == 0.5:
                            plot_colour = 'dodgerblue'
                            outline = 'dodgerblue'
                        elif vaccination_here == 0.8:
                            plot_colour = 'navy'
                            outline = 'navy'
                        elif vaccination_here==0:
                            plot_colour = "white"
                            outline ='lightskyblue'
                    else:
                        if vaccination_here == 0.2:
                    ...
                    if third_vaccination_type=="no further vaccination":
                        alpha = 0.5
                        linecolour = "black"
                    elif third_vaccination_type== "reactive (delayed) vaccination":
                        alpha = 0.8
                        linecolour = plot_colour

                    ax.hist(total_infections_wave_3[third_vaccination_type], bins=10, alpha=alpha, color=plot_colour,label=third_vaccination_type,edgecolor=linecolour,linewidth=0.01) # 

                    ax2.hist(total_severe_disease_wave_3[third_vaccination_type],bins=50, alpha=alpha, color=plot_colour,label=third_vaccination_type,edgecolor=linecolour,linewidth=0.2)
                ...
                leg = ax.legend(bbox_to_anchor=(1.01,0), loc="lower left",borderaxespad=0,frameon=False,title=str(vaccination_here*100)+"\% vaccination coverage")
                leg._legend_box.align = "left"
                ...
                plt.savefig(os.path.join(folder, "wave_3_infections_and_" + ICU_or_death+"_histogram_"+"_".join(younger_or_older)+ "_"+str(vaccination_here)+ "_maxTP_"+str(max(local_TP_list)) + ".png") , bbox_inches='tight')
                plt.close()
...
# plot_ribbon_infections_over_time(younger_or_older=["younger"],immune_escape_wave_start=3,seeding_or_continuous="seeding",with_vaccination_hatch=True)
# plot_ribbon_infections_over_time(younger_or_older=["older"],immune_escape_wave_start=3,seeding_or_continuous="seeding",with_vaccination_hatch=True)

# plot_ribbon_infections_over_time_plus(younger_or_older=["younger"],immune_escape_wave_start=3,seeding_or_continuous="seeding")
# plot_ribbon_infections_over_time_plus(younger_or_older=["older"],immune_escape_wave_start=3,seeding_or_continuous="seeding")

# total_infections_and_deaths_histograms(ICU_or_death='death',younger_or_older=["younger"])
# total_infections_and_deaths_histograms(ICU_or_death='death',younger_or_older=["older"])
...
# plot_ribbon_infections_over_time(younger_or_older=["younger"],immune_escape_wave_start=2,seeding_or_continuous="seeding",with_vaccination_hatch=True)
# plot_ribbon_infections_over_time(younger_or_older=["older"],immune_escape_wave_start=2,seeding_or_continuous="seeding",with_vaccination_hatch=True)

# plot_ribbon_infections_over_time_plus(younger_or_older=["younger"],immune_escape_wave_start=2,seeding_or_continuous="seeding")
# plot_ribbon_infections_over_time_plus(younger_or_older=["older"],immune_escape_wave_start=2,seeding_or_continuous="seeding")
...
# plot_comparison_third_wave_ICU_and_deaths_given_scenario_wavestart(folder_start_wave_2= '/scratch/cm37/tpl/reduced_covid_BA1_BA45_wave_start_2_outputs/',presim_parameters_folder_start_wave_2=presim_parameters_folder ,folder_start_wave_3=  '/scratch/cm37/tpl/reduced_covid_BA1_BA45_wave_start_3_outputs/',presim_parameters_folder_start_wave_3=presim_parameters_folder , ICU_or_death="death",population_type_list = ["younger"])

# plot_comparison_third_wave_ICU_and_deaths_given_scenario_wavestart(folder_start_wave_2= '/scratch/cm37/tpl/reduced_covid_BA1_BA45_wave_start_2_outputs/',presim_parameters_folder_start_wave_2=presim_parameters_folder ,folder_start_wave_3=  '/scratch/cm37/tpl/reduced_covid_BA1_BA45_wave_start_3_outputs/',presim_parameters_folder_start_wave_3=presim_parameters_folder , ICU_or_death="death",population_type_list = ["older"])
...
# plot_ribbon_infections_over_time_plus(younger_or_older=["younger"],immune_escape_wave_start=2,seeding_or_continuous="continuous")
# plot_ribbon_infections_over_time_plus(younger_or_older=["older"],immune_escape_wave_start=2,seeding_or_continuous="continuous")
...
# plot_comparison_third_wave_ICU_and_deaths_given_scenario_wavestart(folder_start_wave_2= '/scratch/cm37/tpl/reduced_cont_intros_covid_BA1_BA45_wave_start_2_outputs/',presim_parameters_folder_start_wave_2=presim_parameters_folder ,folder_start_wave_3=  '/scratch/cm37/tpl/reduced_cont_intros_covid_BA1_BA45_wave_start_3_outputs/',presim_parameters_folder_start_wave_3=presim_parameters_folder , ICU_or_death="death",population_type_list = ["older"])
...
# plot_comparison_third_wave_ICU_and_deaths_given_scenario_wavestart_TP(folder_start_wave_2='/scratch/cm37/tpl/reduced_covid_BA1_BA45_wave_start_2_outputs/',presim_parameters_folder_start_wave_2=presim_parameters_folder ,folder_start_wave_3= '/scratch/cm37/tpl/reduced_covid_BA1_BA45_wave_start_3_outputs/',presim_parameters_folder_start_wave_3=presim_parameters_folder , ICU_or_death="death",population_type_list = ["younger"])

# plot_comparison_third_wave_ICU_and_deaths_given_scenario_wavestart_TP(folder_start_wave_2='/scratch/cm37/tpl/reduced_covid_BA1_BA45_wave_start_2_outputs/',presim_parameters_folder_start_wave_2=presim_parameters_folder ,folder_start_wave_3= '/scratch/cm37/tpl/reduced_covid_BA1_BA45_wave_start_3_outputs/',presim_parameters_folder_start_wave_3=presim_parameters_folder , ICU_or_death="death",population_type_list = ["older"])


################################################################################################
# PLOTTING Waves: 0.5 million and 10 million populations
################################################################################################
WAVESTART_list  = [2,3]

total_population_mil_list = [0.5,10]
# total_population_mil= 0.5
for total_population_mil in total_population_mil_list:
    for WAVESTART in WAVESTART_list:
