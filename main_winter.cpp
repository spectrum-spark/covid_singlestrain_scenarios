/**
 * @file main_winter.cpp
 * @author your name (you@domain.com)
 * @brief winter simulation
 * @version 0.1
 * @date 2022-04-21
 *
...
int main(int argc, char *argv[])
{
    ...
    double t = 0.0;
    double t_winter = sim_params_json["t_winter"];
    ...
    double TPprewinter = sim_params_json["TPprewinter"]; //   "TPprewinter": 0.0,
    ...
    double beta_scale_prewinter = TPprewinter / (sum_expression * ((5.1 - 2.5) + 1.5));
    ...
    std::vector<double> beta_prewinter = alpha;
    std::cout << "beta prewinter " << std::endl;
    for (auto &x : beta_prewinter)
    {
        // Scale so that it is the appropriate size.
        x = beta_scale_prewinter * x;
        std::cout << x << std::endl;
    }
    std::cout << std::endl;
    ...
    disease_model covid_prewinter(beta_prewinter, q, xi, contact_matrix, b, w, neuts_json);
    ...
    while (t < t_end)
    {
        ...
        if (t <= t_winter)
        {
            int prewintercluster_ref = -1;
            for (int i = 0; i < residents.size(); ++i)
            {
                Individual &person = residents[i];
                size_t infected_or_not = residents[i].number_infections;
                if (infected_or_not > 0)
                {                             // aka infection in the prewinter stage:
                    double epsilon = 0.00001; // hey this is inefficient but OH WELL
                    if (fabs(t - residents[i].time_past_infection) <= epsilon * fabs(t))
                    {
                        if (residents[i].covid.infection_status != 'E' && residents[i].covid.infection_status != 'I')
                        {
                            covid.expose_individual(residents[i], t); // resident has become infected
                            residents[i].covid.cluster_number = prewintercluster_ref;
                            E_ref.push_back(i); // Start tracking them.
        ...
        if (t < t_winter)
        {
            t = covid_prewinter.covid_ascm(residents, age_matrix, t, t + vaccination_dt,
