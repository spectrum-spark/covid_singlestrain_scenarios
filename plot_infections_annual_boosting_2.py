...
def plot_ribbon_infections_over_time_plus_fixed_boosting_group(boosting_group_here = "65+",younger_or_older=["older"],immune_escape_time=546):
    ...
    for local_TP_list in [TP_high]:
        for population_type in younger_or_older:
            ...
            upper_ribbon = [max([all_curves_over_local_days[simnum][i] for simnum in range(len(all_curves_over_local_days))]) for i in range(len(local_days))]
            lower_ribbon =   [min([all_curves_over_local_days[simnum][i] for simnum in range(len(all_curves_over_local_days))]) for i in range(len(local_days))]
            median_line =  [np.median([all_curves_over_local_days[simnum][i] for simnum in range(len(all_curves_over_local_days))]) for i in range(len(local_days))]


            ax.fill_between(local_days,upper_ribbon,lower_ribbon,facecolor=many_boosters_colour,alpha=0.1)
            
            plot_colour = many_boosters_colour
            edge_colour = many_boosters_colour

            ax.plot(local_days,median_line,color = plot_colour,linestyle='solid')
