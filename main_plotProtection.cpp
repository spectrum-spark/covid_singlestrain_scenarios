...
int main(int argc, char *argv[])
{
  ...
  // Load VE parameters
  std::string ve_params_filename(argv[3]);
  nlohmann::json ve_params_json = load_json(ve_params_filename);
  ...
  std::string vaccination_scenario_foldername(argv[5]); // Reads in scenario name.
  std::string vaccination_scenario_name(argv[6]);       // Reads in scenario name.
  ...
  disease_model covid(beta, q, xi, contact_matrix, b, w, ve_params_json);
  ...
  if (output_age_file.is_open())
  {
    ...
    for (int i = 0; i < ageProtectionInfection.size(); ++i)
    {
      ...
      {
        output_age_file << tout[i] << ", " << j << ", " << ageProtectionInfection[i][j] << ", "
                        << "Acquisition, " << sim_number << "\n";
        output_age_file << tout[i] << ", " << j << ", " << ageProtectionSymptoms[i][j] << ", "
                        << "Symptoms, " << sim_number << "\n";
        output_age_file << tout[i] << ", " << j << ", " << ageProtectionOnwards[i][j] << ", "
                        << "Onwards transmission, " << sim_number << "\n";
        output_age_file << tout[i] << ", " << j << ", " << ageOverallReduction[i][j] << ", "
                        << "Overall reduction, " << sim_number << "\n";
      }
    }
