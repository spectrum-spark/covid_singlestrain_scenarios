    ...
    for vaccination_coverage in [0.2,0.5]:
        ...
        for local_TP_list in TP_segregated_list:
            ...
            for population_type in younger_or_older:
                ...
                fig, ((ax2,ax_table),(ax3,ax5),(ax4,ax6)) = plt.subplots(3,2,gridspec_kw={'height_ratios': [6, 1,1],'width_ratios':[3,4]}, figsize=(14,4))
...
#             leg = Legend(ax, legend_points,boosters_only_vaccination_start_list,title=younger_or_older[0] +" population, high risk boosting; boosting times:",bbox_to_anchor=(1.01,0), loc="lower left",borderaxespad=0,frameon=False)
#             leg._legend_box.align = "left"
#             ax.add_artist(leg)
...
# def total_infections_and_deaths_histograms_fixed_boosting_group(boosting_group_here = "65+",ICU_or_death='death',younger_or_older=["older"],timeframe =local_days):
#     boosters_start_colour_list = ['white','salmon','red','firebrick']
#     boosters_start_edgecolour_list = ['salmon','none','none','none']
...
#                         for aug in range(1,aug_num+1):
#                             new_pd_ICU = clinical_pd_obj.loc[(clinical_pd_obj['iteration']==(simnum-1)*aug_num+aug) & (clinical_pd_obj['day'].isin(timeframe ))]
...
#             plt.savefig(os.path.join(folder, "total_infections_and_" + ICU_or_death+"_histogram_"+population_type+ "_fixed_boosting_group"+"_boosting_"+str(boosting_group_here)+ "_maxTP_"+str(max(local_TP_list)) +"_time"+str(timeframe[0])+"-" + str(timeframe[-1])+ ".png") , bbox_inches='tight')
#             plt.close()
...
for immune_escape_time in immune_escape_times:
    ...
    total_deaths_histograms(original_program_time + 26*7 ,immune_escape_time,ICU_or_death='death',younger_or_older=["younger"],timeframe=list(range(original_program_time,max_days)))
