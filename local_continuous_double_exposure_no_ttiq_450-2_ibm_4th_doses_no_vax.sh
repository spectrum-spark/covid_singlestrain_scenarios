...
do
    ...
    do 
        ...
        do
            for ((TPvers = 1; TPvers<=21;TPvers++))
            do
                ./RunContinuousDoubleExposureNoTTIQibm4thdoses updated_omicron_parameters/omicron_May_2022_updates.json winter_scenarios_continuous_double_exposure_no_ttiq_450-2_ibm_4th_doses_no_vax/continuous_${POP}_${TPvers}_local.json $i C:\\Users\\thaophuongl\\covid-abm-presim\\continuous_sim_param_files_no_vax abm_continuous_simulation_parameters_${POP}_${diffparams}
            done
        done
    done
done
