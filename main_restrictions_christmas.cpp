...
int main(int argc, char* argv[]) {
  ...
  std::string vaccination_scenario_foldername(
  ...
  std::string folder =
      vaccination_scenario_name + (std::string)sim_params_json["folder_suffix"];
  ...
  std::vector<std::uniform_real_distribution<double>> generate_age =
      read_age_generation(vaccination_scenario_foldername + "/dim_age_band.csv",
                          100.0);

  // Create residents.
  std::vector<double> age_brackets = sim_params_json["age_brackets"];
  std::vector<Individual> residents =
      read_individuals(vaccination_scenario_foldername + "/" +
                           vaccination_scenario_name + ".csv",
  ...
  double TP_christmas = sim_params_json["TP_christmas"];

  double beta_scale =
      TP / (sum_expression *
            ((5.1 - 2.5) +
             1.5));  // This is hardcoded, be careful if anything changes.
  double beta_restriction_scale =
      (TP * TP_restrictions) / (sum_expression * ((5.1 - 2.5) + 1.5));

  double christmas_restriction_scale =
      (TP * TP_christmas) / (sum_expression * ((5.1 - 2.5) + 1.5));
  ...
  std::vector<double> christmas_restrictions = alpha;
  std::cout << "christmas restrictions " << std::endl;
  for (auto& x : christmas_restrictions) {
    // Scale so that it is the appropriate size.
    x = christmas_restriction_scale * x;
    std::cout << x << std::endl;
  }
  std::cout << std::endl;
  ...
  disease_model covid_restrictions(beta_restrictions, q, xi, contact_matrix, b,
                                   w, neuts_json);
  disease_model covid_christmas(christmas_restrictions, q, xi,
                                   contact_matrix, b, w, neuts_json);

  double start_restrictions = sim_params_json["start_restrictions"];
  double start_christmas = sim_params_json["start_christmas"];
