...
load([broad_scenario_name,'/sim_number_',num2str(1),'_',num2str(rel_sev),'_agestrat_discrete.mat']);
BIG_data_frame = zeros(num_sims*size(data_frame,1),size(data_frame,2));
for ii = 1:num_sims
    %this loads the data frame
    load([broad_scenario_name,'/sim_number_',num2str(ii),'_',num2str(rel_sev),'_agestrat_discrete.mat']);
    data_frame(:,2) = data_frame(:,2) + aug_num*(ii-1);
    BIG_data_frame((1:size(data_frame,1))+(ii-1)*size(data_frame,1),:) = data_frame;
    
end

T = array2table(BIG_data_frame);
T.Properties.VariableNames={'age','iteration','day','daily_total_infections','daily_symptomatic_infections','daily_admissions','ward_occupancy','daily_ICU_admissions','ICU_occupancy','daily_deaths'};
writetable(T,[broad_scenario_name,'_full_outcomes_dataframe','.csv'])
...
%
%save([broad_scenario_name,'_full.mat'])%,all_infections,new_symp_series,new_admission_series,new_ICU_series,ward_OCC_series,ICU_OCC_series,daily_deaths,daily_discharges)
