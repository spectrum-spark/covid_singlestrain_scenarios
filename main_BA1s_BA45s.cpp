...
int main(int argc, char *argv[])
{
    ...
    double new_strain_time =sim_params_json["new_strain_time"];
    ...
    // Seed the new infections.
  int new_strain_total_initial_infected =
      getJsonValue<int>(sim_params_json, "new_strain_initial_infections");
...
//   }
  ...
  // New strain emergence.
  // in Eamon's version, the BA2 code was run (aka uncommented) and so the ratio was relative to BA2; for us, different? aka more hypothetical?
  for (auto& x : beta) {
  ...
  disease_model new_covid(beta, q, xi, contact_matrix, b, w, new_strain_neuts_json,
    ...
    std::vector<VaccinationSchedule> second_booster_doses; // in case we want 2nd booster doses in the code soon
